...
# Define a function that splits sentences into words, and strips
# punctuation marks (",", ";", etc.) from edges of words.
...
# Use the last two functions in one that takes a filename as its argument
# and returns the text in the file as a list of lists. Test it on the file
...
# ### 4.1.2
# Use the functions defined in __4.1.1__ and define a function that goes
# through a text and replaces all proper names (capitalized words not at
# the beginning of a sentence) with "Joe". Print the first few sentences
...
# ### 4.1.3
# Load the sample text using your function from __4.1.1__ and create a
# game where the user is shown a half of a word in a small context (e.g.
# "_Many solu\*\*\*\*\* were suggested_") and has to guess the full word
# (don't worry about randomization, your solution can come up with the
...
# ### 4.2.1
# Define a function that takes as its input a list of $n$ lists of $n$
# numbers (a square matrix) and decides if it is symmetric (i.e. $A[i,j]
# == A[j,i]$ for all $i, j$).
...
# (i.e. a table of size $n\times k$) and "transposes" it, creating a table
# of size $k\times n$.
...
# Define a function that takes a list and string, then returns all
# elements that start with the string, along with their indices in the
# list.
...
# ### 4.3.1
# Use a dictionary to count words in our sample text (use your text
# processing functions!). Then print the most common words, along with
# their frequencies!


# ### 4.3.2

# Define function that performs the factorial operation ($n!$) but caches
# all results so that each call requires the least possible number of
# multiplications.


# ### 4.3.3
# Read the dataset in "data/movies.tsv" and store it in a dictionary whose
...
# ### 4.3.4
# Process the movies dataset (the original file or the dictionary built in
# __4.3.3__) and build a dictionary that indexes movies by the first
# letter of the title. Then create a small interface for querying (using
# the input function)


# ### 4.3.5
# Build an incremental search of movie titles: users should be able to
# narrow the set of movies with every character they type. You may create
# deeply nested dictionaries beforehand or process the data on-the-fly.
...
def search(fn):
    data = [(title.strip(), int(year), genres.split(','))
            for title, year, genres in [line.strip().split('\t') for line in open(fn)]]
...
# ### 4.4.3
# Define a function that queries users for their last name, first name,
# year of birth, and hobby, and populates an OrderedDict whose keys are
# the last names and values are dictionaries with four keys each. If a
# second person with the same last name is encountered, both should now
# have keys of the form "lastname_firstname". If the same person is
# encountered multiple times, his/her data should be updated. Then test
# the solution of someone else and ask her to test yours.
